...
Update your local database connection and SMTP password in `application.properties` or create your own `application-local.properties` file to override
...
```
java -Dspring.profiles.active=production -jar ./target/scl-0.0.1-SNAPSHOT.jar
```
...
```
mvnw spring-boot:build-image -Dspring-boot.build-image.imageName=ludo.mentis.aciem/scl
```
